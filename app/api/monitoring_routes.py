...
async def record_counter_metric(request: MetricRequest) -> Dict[str, str]:
    ...
    try:
        ...
        return {
            "message": f"Counter metric '{request.name}' recorded successfully"
        }
    except Exception as e:
        raise HTTPException(
...
async def record_gauge_metric(request: MetricRequest) -> Dict[str, str]:
    ...
    try:
        ...
        return {
            "message": f"Gauge metric '{request.name}' recorded successfully"
        }
    except Exception as e:
        raise HTTPException(
            status_code=status.HTTP_400_BAD_REQUEST,
...
async def record_histogram_metric(request: MetricRequest) -> Dict[str, str]:
    ...
    try:
        metrics_collector.record_histogram(
            request.name, request.value, request.tags
        )
        return {
            "message": (
                f"Histogram metric '{request.name}' recorded successfully"
            )
        }
    except Exception as e:
        raise HTTPException(
...
async def monitoring_health_check() -> Dict[str, Any]:
    ...
    try:
        ...
        critical_alerts = [
            alert for alert in alerts if alert["level"] == "critical"
        ]
        warning_alerts = [
            alert for alert in alerts if alert["level"] == "warning"
        ]
