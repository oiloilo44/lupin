...
async def get_game_config_by_name(game_name: str) -> Dict[str, Any]:
    ...
    try:
        ...
        # 런타임 오버라이드 적용
        return runtime_manager.apply_overrides_to_config(
            base_config, game_name
        )
    except Exception as e:
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
...
async def update_server_config(request: ConfigUpdateRequest) -> Dict[str, str]:
    ...
    if success:
        ...
        return {
            "message": (
                f"서버 설정 업데이트 성공: {request.path} = {request.value}"
            )
        }
    else:
        raise HTTPException(
