...
from .metrics import get_metrics_collector, MetricsCollector, GaugeMetric
...
class PerformanceMonitor:
    ...
    def timer(self, name: str, tags: Optional[Dict[str, str]] = None):
        ...
        try:
            ...
            self.metrics.record_histogram(f"{name}.duration", duration * 1000, tags)  # ms 단위
    ...
    def async_timed(self, name: Optional[str] = None, tags: Optional[Dict[str, str]] = None):
    ...
    def count_calls(self, name: Optional[str] = None, tags: Optional[Dict[str, str]] = None):
        ...
        def decorator(func: Callable[P, T]) -> Callable[P, T]:
            ...
            def wrapper(*args: P.args, **kwargs: P.kwargs) -> T:
                try:
                    ...
                    self.metrics.increment_counter(f"{metric_name}.error", 1, error_tags)
                    raise
            
            return wrapper
        return decorator
    
    def async_count_calls(self, name: Optional[str] = None, tags: Optional[Dict[str, str]] = None):
    ...
    def monitor_memory_usage(self, name: str, tags: Optional[Dict[str, str]] = None) -> None:
        """메모리 사용량 모니터링"""
        try:
            import psutil
            ...
            self.metrics.set_gauge(f"{name}.memory.rss", memory_info.rss / 1024 / 1024, tags)  # MB
            # VMS (Virtual Memory Size) - 가상 메모리 사용량  
            self.metrics.set_gauge(f"{name}.memory.vms", memory_info.vms / 1024 / 1024, tags)  # MB
    ...
    def monitor_cpu_usage(self, name: str, tags: Optional[Dict[str, str]] = None) -> None:
        """CPU 사용량 모니터링"""
        try:
            import psutil
            process = psutil.Process()
        ...
        for category in ['histograms', 'counters', 'gauges']:
            ...
            for key, value in all_metrics[category].items():
                ...
                if any(keyword in key for keyword in [
                    'duration', 'calls', 'memory', 'cpu', 'latency', 'response_time'
                ]):
                    performance_metrics[category][key] = value
        
        return performance_metrics
    
    def record_event_processing_time(self, event_type: str, duration_ms: float, 
                                   success: bool = True, tags: Optional[Dict[str, str]] = None) -> None:
        """이벤트 처리 시간 기록"""
        base_tags = {"event_type": event_type, "status": "success" if success else "error"}
        if tags:
    ...
    def record_websocket_metrics(self, event: str, connection_count: Optional[int] = None,
    ...
    def record_game_session_metrics(self, game_type: str, session_duration_seconds: float,
                                   players_count: int, tags: Optional[Dict[str, str]] = None) -> None:
        """게임 세션 메트릭 기록"""
        ...
        # 세션 지속 시간
        self.metrics.record_histogram("game.session_duration", session_duration_seconds, base_tags)
        
        # 세션 카운트
        self.metrics.increment_counter("game.sessions", 1, base_tags)
    
    def record_game_session_start(self, game_type: str, room_id: str, player_count: int,
                                tags: Optional[Dict[str, str]] = None) -> None:
        """게임 세션 시작 메트릭 기록"""
        base_tags = {"game_type": game_type, "room_id": room_id, "players": str(player_count)}
        if tags:
            base_tags.update(tags)
        ...
        # 현재 활성 세션 수 증가
        current_sessions = self.metrics._gauges.get("game.active_sessions", GaugeMetric("game.active_sessions", 0)).value
        self.metrics.set_gauge("game.active_sessions", current_sessions + 1, {"game_type": game_type})
    
    def record_game_session_end(self, game_type: str, room_id: str, winner: Optional[str] = None,
                               reason: str = "normal", tags: Optional[Dict[str, str]] = None) -> None:
        """게임 세션 종료 메트릭 기록"""
        ...
        # 현재 활성 세션 수 감소
        current_sessions = self.metrics._gauges.get("game.active_sessions", GaugeMetric("game.active_sessions", 1)).value
        self.metrics.set_gauge("game.active_sessions", max(0, current_sessions - 1), {"game_type": game_type})
