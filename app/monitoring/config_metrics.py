...
class ConfigMetrics:
    ...
    def record_config_change(self, config_name: str, path: str, old_value: Any, 
                           new_value: Any, changed_by: Optional[str] = None,
                           source: str = "api", tags: Optional[Dict[str, str]] = None) -> None:
        """설정 변경 기록"""
        change = ConfigChange(
        ...
        # 설정별 변경 카운트
        self.metrics.increment_counter(f"config.{config_name}.changes", 1, 
                                     {k: v for k, v in metric_tags.items() if k != "config_name"})
        
        logger.info(f"Config change recorded: {config_name}.{path} = {new_value} "
                   f"(was: {old_value}) by {changed_by or 'system'}")
    
    def record_config_load_time(self, config_name: str, load_time_ms: float,
                              source: str = "file", tags: Optional[Dict[str, str]] = None) -> None:
        """설정 로드 시간 기록"""
        metric_tags = {
            "config_name": config_name,
            "source": source,
    ...
    def record_config_validation_result(self, config_name: str, success: bool,
    ...
    def record_config_cache_metrics(self, cache_hits: int, cache_misses: int,
                                   cache_size: int, tags: Optional[Dict[str, str]] = None) -> None:
        """설정 캐시 메트릭 기록"""
        base_tags = tags or {}
    ...
    def get_config_change_history(self, config_name: Optional[str] = None,
    ...
    def get_config_health_score(self) -> Dict[str, Any]:
        ...
        # 캐시 히트율 확인
        cache_gauges = {k: v for k, v in all_metrics.get("gauges", {}).items() 
        ...
        # 에러율 확인
        validation_counters = {k: v for k, v in all_metrics.get("counters", {}).items() 
        ...
        error_rate = (error_validations / total_validations * 100) if total_validations > 0 else 0
        ...
        # 캐시 히트율이 낮으면 점수 감점
        if hit_rate < 80:
            health_score -= (80 - hit_rate) * 0.5
        
        # 에러율이 높으면 점수 감점
        if error_rate > 5:
            health_score -= (error_rate - 5) * 2
