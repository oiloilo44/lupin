...
class MonitoringDashboard:
    ...
    def get_alerts(self) -> List[Dict[str, Any]]:
        ...
        if overview["system_health"]["error_rate"] > 5:
            alerts.append({
                "level": "warning" if overview["system_health"]["error_rate"] < 10 else "critical",
                "message": f"High error rate: {overview['system_health']['error_rate']:.2f}%",
                "metric": "error_rate",
        ...
        if overview["performance"]["avg_response_time_ms"] > 1000:
            alerts.append({
                "level": "warning" if overview["performance"]["avg_response_time_ms"] < 2000 else "critical",
                "message": f"High response time: {overview['performance']['avg_response_time_ms']:.2f}ms",
    ...
    def _get_top_slow_endpoints(self, response_times: Dict[str, Any], limit: int = 5) -> List[Dict[str, Any]]:
        """가장 느린 엔드포인트들"""
        endpoints = []
    ...
    def _get_top_error_endpoints(self, error_rates: Dict[str, Any], limit: int = 5) -> List[Dict[str, Any]]:
        """에러율이 가장 높은 엔드포인트들"""
        endpoints = []
