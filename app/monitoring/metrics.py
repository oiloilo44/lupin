...
class MetricsCollector:
    ...
    def get_counter(self, name: str, tags: Optional[Dict[str, str]] = None) -> Optional[CounterMetric]:
        """카운터 조회"""
        key = self._make_key(name, tags or {})
        with self._lock:
            return self._counters.get(key)
    
    def get_gauge(self, name: str, tags: Optional[Dict[str, str]] = None) -> Optional[GaugeMetric]:
        """게이지 조회"""
        key = self._make_key(name, tags or {})
        with self._lock:
            return self._gauges.get(key)
    
    def get_histogram(self, name: str, tags: Optional[Dict[str, str]] = None) -> Optional[HistogramMetric]:
        """히스토그램 조회"""
        key = self._make_key(name, tags or {})
        with self._lock:
    ...
    def _add_time_series_point(self, key: str, value: float, tags: Dict[str, str]) -> None:
        """시계열 데이터 포인트 추가"""
        point = MetricPoint(
        ...
        while (self._time_series[key] and 
               self._time_series[key][0].timestamp < cutoff):
