...
from ..models import Room, GameType, Player, GameMove, OmokGameState
...
class OmokManager(BaseGameManager):
    ...
    def validate_move(self, room: Room, player: Player, x: int, y: int) -> Tuple[bool, str]:
        """이동 유효성 검증"""
        # 게임 종료 확인
        if room.game_ended:
    ...
    def make_move(self, room: Room, player: Player, x: int, y: int) -> Tuple[bool, Optional[List[dict]], str]:
        """이동 실행 및 승리 조건 확인
