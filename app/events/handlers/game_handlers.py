...
class GameEventHandler:
    ...
    def handle_player_joined(self, event: PlayerJoinedEvent) -> None:
        ...
        if len(event.room.players) >= 2 and event.room.status == GameStatus.WAITING:
            logger.debug(f"Room {event.room_id} ready to start (players: {len(event.room.players)})")
            
        # 재접속이 아닌 경우 플레이어 통계 업데이트
        if not event.is_rejoining:
            self.game_statistics["total_players"] += 1
            logger.debug(f"Total players count: {self.game_statistics['total_players']}")
    ...
    def handle_player_left(self, event: PlayerLeftEvent) -> None:
        ...
        if event.room.status == GameStatus.PLAYING:
            logger.info(f"Player left during game in room {event.room_id}, pausing game")
    ...
    def handle_game_ended(self, event: GameEndedEvent) -> None:
        ...
        if total_games > 0:
            new_avg = ((current_avg * (total_games - 1)) + event.game_duration) / total_games
        ...
        logger.info(
            ...
            f"Average duration: {self.game_statistics['average_game_duration']:.1f}s"
        )
...
def register_game_handlers(event_bus: Any, room_timer: Any = None) -> GameEventHandler:
    ...
    event_bus.subscribe(RoomCleanupScheduledEvent, handler.handle_room_cleanup_scheduled, priority=80)
