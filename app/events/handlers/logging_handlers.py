...
class LoggingEventHandler:
    ...
    def handle_invalid_move_attempted(self, event: InvalidMoveAttemptedEvent) -> None:
        """유효하지 않은 수 시도 로깅"""
        logger.warning(
            f"Invalid move attempted by {event.player.nickname} in room {event.room_id}: "
            f"{event.move_data} - Reason: {event.error_reason}"
        )
    
    def handle_room_created(self, event: RoomCreatedEvent) -> None:
        """방 생성 로깅"""
        logger.info(
            f"Room created: {event.room_id} (type: {event.game_type.value})"
        )
    
    def handle_room_deleted(self, event: RoomDeletedEvent) -> None:
        """방 삭제 로깅"""
        logger.info(
            f"Room deleted: {event.room_id} (reason: {event.reason})"
        )
    ...
    def handle_chat_message(self, event: ChatMessageEvent) -> None:
        ...
        logger.debug(
            f"Chat message in room {event.room_id} from {event.player.nickname}: "
            f"{event.message[:max_length]}{'...' if len(event.message) > max_length else ''}"
        )
    
    def handle_global_event(self, event: GameEvent) -> None:
        """모든 이벤트에 대한 글로벌 로깅 및 성능 메트릭"""
        event_name = type(event).__name__
        logger.debug(f"Event: {event_name} in room {event.room_id} at {event.timestamp}")
...
def register_logging_handlers(event_bus: Any) -> None:
    ...
    event_bus.subscribe(PlayerDisconnectedEvent, handler.handle_player_disconnected, priority=100)
    event_bus.subscribe(PlayerReconnectedEvent, handler.handle_player_reconnected, priority=100)
    ...
    event_bus.subscribe(InvalidMoveAttemptedEvent, handler.handle_invalid_move_attempted, priority=100)
