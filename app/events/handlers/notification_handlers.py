...
from ..game_events import (GameEndedEvent, GameStartedEvent,
...
def register_notification_handlers(
    event_bus: Any, get_connections_func: Callable[[str], Set[WebSocket]]
) -> None:
    """알림 핸들러들을 이벤트 버스에 등록
    ...
    event_bus.subscribe(
        PlayerDisconnectedEvent, handler.handle_player_disconnected, priority=50
    )
    event_bus.subscribe(
        PlayerReconnectedEvent, handler.handle_player_reconnected, priority=50
    )
    ...
    event_bus.subscribe(
        RestartRequestedEvent, handler.handle_restart_requested, priority=50
    )
    event_bus.subscribe(
        RestartAcceptedEvent, handler.handle_restart_accepted, priority=50
    )
